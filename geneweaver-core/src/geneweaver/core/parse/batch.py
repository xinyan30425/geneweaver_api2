"""
is_batch_file：此函数用于检查上传的文件是否符合GeneWeaver批处理文件的格式。根据文件内容判断文件类型，以便正确处理。

process_lines：用于解析文件中的每一行，创建基因集实例列表，这些实例随后可以用于程序中的批量上传或处理。

标题处理功能（read_header, update_header等）：这些函数用于读取和解析文件中的标题信息，标题信息对于理解基因集的元数据是必要的。

值处理功能（read_values, process_value_line）：这些函数用于读取和解析文件中代表基因集内容的行。

create_geneset：用于将读取到的标题和值信息合并，创建一个基因集对象，该对象可以在GeneWeaver中使用。

finalize_processed_geneset：在文件的每个基因集部分结束时调用，以确保所有数据都被正确处理并保存。

异常检查功能（如string_has_newlines）：用于在处理文件时确保数据的格式正确，没有不符合要求的换行等情况。

这些函数的组合使用，使得GeneWeaver能够有效地处理和解析大量基因集数据，进而进行批量分析和管理


这个模块提供了一组用于处理批量基因集文件及其内容的函数。包括检查文件类型、处理内容行、读取标题和值、更新和重置标题、以及为批量上传创建基因集对象的功能。

文件类型识别功能：
- is_batch_file：检查提供的文件内容是否为有效的批处理文件。

行处理功能：
- process_lines：处理每一行内容，构建并返回一系列BatchUploadGeneset实例。

标题处理功能：
- read_header：处理标题行，并相应地更新状态变量。
- update_header：根据处理行中的键值对更新标题字典。
- reset_required_header_values：重置标题字典中的必需值。
- check_has_required_header_values：检查标题字典是否具有必需的值。
- process_header_line：将标题行处理成键值对。
- read_single_prefix_header：将单前缀标题行读取成键值对。
- read_space_separated_header：将空格分隔的标题行读取成键值对。

值处理功能：
- read_values：假设某行是值，并在必要时更新阅读模式。
- process_value_line：将值行处理成键值对。

基因集构建功能：
- create_geneset：从批量上传数据创建一个基因集对象。

基因集最终确定功能：
- finalize_processed_geneset：将当前基因集添加到列表中，并准备处理下一个。

异常检查功能：
- string_has_newlines：检查字符串中是否有换行符。

异常：
- UnsupportedFileTypeError：如果文件既不是批处理文件也不是基因集值文件，则抛出。
- MultiLineStringError：如果在值行中遇到带有回车字符的情况，则抛出。
- InvalidBatchValueLineError：如果在值行中遇到超过两个值的情况，则抛出。
- NotAHeaderRowError：当预期标题行未找到时抛出。
- MissingRequiredHeaderError：如果在标题中缺少任何必需的键，则抛出。
- IgnoreLineError：如果根据其前缀应忽略该行，则抛出。
